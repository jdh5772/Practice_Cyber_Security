...
- `'`를 사용했을 때 오류가 난다는 것은 추가적으로 `'`가 붙여졌다는 것. 스택쿼리를 사용해볼 것.
- `' AND 1=2 UNION SELECT 1,2,3,4,5,6;-- -` : `UNION SELECT` 인젝션을 테스트할 때 앞을 false로 만들어서 더 안정적으로 시도.
- 어떤 DB를 사용하는지 모를 때는 mssql 쿼리를 시도해봐야 한다.
