...
- http : `robots.txt`/`gobuster`/`feroxbuster`/`ffuf`/`error page`/`cookie에 따라서 redirection`/`.git`/`configuration file location`/`link`/`서버 포트가 다르더라도 다른 포트에서 사용하는 언어에 맞는 web shell을 업로드 해볼 것.`/`burpsuite가 실패하는 경우가 있으니 curl로 시도해볼 것.`/`특별한 정보 찾을 수 없을 때 api 검색(login api)`
- https : `tls 인증서 확인`
