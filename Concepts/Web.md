...
---
## JWT / JWS / JWE
- `JWT` : JSON 데이터를 안전하게 전달하기 위한 토큰 포맷
- `JWS` : 비밀키로 단방향 해시화 → 변조 방지 (Payload는 누구나 읽기 가능)
- `JWE` : 비밀키로 암호화 → 복호화 가능, 내용 기밀 유지
- 서버에서 Cookie / Session Storage / Local Storage 등 어떤 방식으로 로그인 되는지 살펴봐야 한다.

| 항목 | JWT | JWS | JWE |
|------|-----|-----|-----|
| 풀네임 | JSON Web Token | JSON Web Signature | JSON Web Encryption |
| RFC | RFC 7519 | RFC 7515 | RFC 7516 |
| 목적 | 인증/인가용 토큰 표준 | **무결성** 보장 (변조 방지) | **기밀성** 보장 (내용 암호화) |
| 핵심 질문 | "이 토큰이 유효한가?" | "누가 만들었고, 안 바뀌었나?" | "내용을 숨길 수 있나?" |
