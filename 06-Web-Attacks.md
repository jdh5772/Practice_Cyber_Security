...
- `hacktricks`에서 각 언어에 맞는 SSTI 페이로드 확인.
...
## PHP ZIP Wrapper
```bash
http://localhost/index.php?page=zip://path/to/file.zip%23shell&cmd=whoami
```
...
- `Response`의 헤더 변경.
<img width="895" height="595" alt="image" src="https://github.com/user-attachments/assets/7b0de2b0-c3dd-4c7f-9748-91ef21aba3ce" />
...
---
<details>
	<summary><strong>Chrome Remote Debugging Port</strong></summary>

- Chrome을 `--remote-debugging-port` 플래그로 실행하면 **Chrome DevTools Protocol(CDP)** 을 통해 브라우저를 원격으로 제어할 수 있다.

## 왜 브라우저 주소창에서 직접 접근이 안 되는가?

### 1. CDP는 WebSocket 기반이다

`--remote-debugging-port`로 열리는 서버는 일반 HTTP 서버가 아니라 **WebSocket 엔드포인트**다.  
브라우저 주소창에서 `localhost:41829`를 입력하면 일반 HTTP 요청을 보내지만, CDP 서버는 WebSocket 업그레이드 요청이 아닌 한 제대로 응답하지 않는다.


## `chrome://inspect`가 작동하는 이유

`chrome://inspect`는 **브라우저 내부 페이지**이기 때문에 Chrome이 직접 CDP 클라이언트 역할을 한다.

```
chrome://inspect
    ↓
내부적으로 HTTP GET localhost:41829/json 요청
    ↓
탭 목록 JSON 파싱 후 UI 표시
    ↓
탭 선택 시 WebSocket 연결 수립
    ws://localhost:41829/devtools/page/{id}
```

즉, `chrome://inspect`는 CDP용 공식 클라이언트 UI다.
